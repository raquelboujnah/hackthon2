    ...
    arr.forEach(recipe => {
        ...
        document.querySelectorAll('.update-button').forEach(button => {
            button.addEventListener('click', (e) => {
                const recipeId = e.target.closest('.onerecipe').getAttribute('data-recipe-id');
...
function openUpdateModal(recipeId) {
    ...
    fetch(`http://localhost:3000/recipes/recipe/${recipeId}`)
        ...
        .then(data => {
            ...
            document.getElementById('update-is-vegan').value = recipe.is_vegan ? 'true' : 'false';

            populateDynamicFields('update-ingredients-list', recipe.ingredients, 'ingredient-update');
            populateDynamicFields('update-steps-list', recipe.description, 'step-update');
        })
...
document.getElementById('add-ingredient-update').addEventListener('click', () => {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("add-ingredient").addEventListener("click", () => {
        ...
        ingredientItem.querySelector(".remove-ingredient").addEventListener("click", (e) => {
...
async function updateRecipe(e){
    ...
    const updatedRecipe = {
        ...
        ingredients: Array.from(document.querySelectorAll('#update-ingredients-list input')).map(input => input.value),
        description: Array.from(document.querySelectorAll('#update-steps-list input')).map(input => input.value),
    };
